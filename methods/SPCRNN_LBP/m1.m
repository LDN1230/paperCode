...
% lambda = [0 0.2 0.4 0.6 0.8 1 1.2 1.4 1.6 1.8 2];
% LambdaOA = zeros(1, length(lambda));
% for t = 1:length(lambda)
                    
%         for K_2 = 9:9
%                    nums = [1000 2000 3000 4000 5000 6000 7000 8000 9000 10000 11000 12000 13000 14000 15000];
                    nums = [16000 17000 18000 19000 20000];
                    SPCRNN_result.OA = zeros(1,15);
                    SPCRNN_result.AA = zeros(1,15);
                    SPCRNN_result.kappa = zeros(1,15);
                    SPCRNN_result.CA = zeros(C, 15);
                    SPCRNNLBP_results.OA = zeros(1,15);
                    SPCRNNLBP_results.AA = zeros(1,15);
                    SPCRNNLBP_results.kappa = zeros(1,15);
                    SPCRNNLBP_results.CA = zeros(15, C);
                    for it = 1:length(nums)
%                         fprintf('第%d次实验\n', it);
                        %产生超像素
                        numSuperpixels = nums(it);
                        fprintf('超像素数目：%d\n', numSuperpixels);
                        ...
                        [distance] = SPCRNN(im_2d, test_data_ori , tt_index, train_data_ori, D_label,superpixel_label, 1,1,index,index_map, 0.5);
