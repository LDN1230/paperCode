function [best_k, knn_test_response]=calculate_best_k(Train_array_response,Train_array)
...
for k=1:2:17
    avg_err=0;
    for i=1:5
        knn_train=Train_array(:,find(indices ~= i),:);
        knn_train_response=Train_array_response(find(indices ~= i));
        knn_test=Train_array(:,find(indices == i),:);
        knn_test_response=Train_array_response(find(indices == i));
