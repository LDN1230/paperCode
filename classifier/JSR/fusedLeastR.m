...
if (opts.lFlag==0)
    ...
    xp=x; Axp=Ax; xxp=zeros(n,size(y,2));
...
%         funVal(iterStep)=Axy'* Axy/2  + sum(abs(x)) * lambda + lambda2 * sum( abs( x(2:n) -x(1:(n-1)) ));
        funVal(iterStep)=sum(sum(Axys,1)) / size(Axy,1)/2  + sum(abs(x)) * 0.5 + 0.5 * sum( abs( x(2:n) -x(1:(n-1)) ));
        if (bFlag)
