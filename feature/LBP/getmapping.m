...
if strcmp(mappingtype,'u2') %Uniform 2
    ...
    for i = 0:2^samples-1
        ...
        i_bin = dec2bin(i,samples);                 %character vector
        j_bin = circshift(i_bin',-1)';              %circularly rotate left循环左移
        ...
        if numt <= 2                                %绝大多数LBP模式最多只包含两次从1到0或从0到1的跳变
            table(i+1) = index;                     %等价模式
            index = index + 1;
        else
            table(i+1) = newMax - 1;                %混合模式
        end
    end
end
