function mergedSuperpixel = mergeSuperpixel_test(originalSuperpixel,feature)
% 超像素融合
            ...
            for i = 1:length(superpixel_label)
                ...
                for j = (i+1):length(superpixel_label)
                    ...
                    if isempty(intersection) == 0  % 有交集则是相邻的
                        ...
                        area1 = length(index1); area2 = length(index2);
                        distance = distance+coefficient*(1-min(area1,area2)/max(area1,area2));
        ...
        if n >1
             ...
             for gg = 1:length(index4)
                ...
                if isempty(intersection) == 0  % 有交集则是相邻的
                 ...
                 G12 = sum(p1.*log(p1))+sum(p2.*log(p2)) + 2*log(2) - sum(p3.*log(p3));
                 distance = G12/(gamma*sharedBoundary);
                 area1 = length(index5); area2 = length(index6);
                 distance = distance+coefficient*(1-min(area1,area2)/max(area1,area2));
                 tempSuperpixelInfo(1,4) = distance;
                 % 计算合并的cost
                 s1 = length(index5);
